...
jQuery( document ).ready( function( $ ) {
	...
	$('textarea.wp-editor-area').replaceWith( function() {
		return '<blockquote class="wp-editor-area">{$tax_info} <hr/>' + $(this).val() + '</blockquote>';
	} );
	...
	protected function get_term_tag_html( $post_id ) {
		...
		if ( count( $terms ) ) {
			foreach ( $terms as $term ) {
				$term_html .= sprintf( '<span class="wds-log-tag" style="%s">%s</span>', $term->description, $term->name );
			}
		}
